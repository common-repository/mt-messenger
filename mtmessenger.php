...
function mt_messenger_settings_link( $links ) {
    $settings_link = '<a href="admin.php?page=mt_messenger_admin">' . __( 'Settings' ) . '</a>';
    array_push( $links, $settings_link );
  	return $links;
}
$plugin = plugin_basename( __FILE__ );
add_filter( "plugin_action_links_$plugin", 'mt_messenger_settings_link' );
...
function mt_messenger_admin_menu() {
	add_menu_page('Messenger',
				  ...
				  'administrator',
				  'mt_messenger_admin',
				  'mt_messenger_backend',
				  plugins_url( 'icon.png', __FILE__ )
				 );
}

// front end, adding div in wp_head goes after body.
add_action('wp_head', 'mt_messenger_frontend');
function mt_messenger_frontend(){
	// file path
...
function mt_messenger_backend(){
  $err = '';
	...
	if(isset($_POST['mt_messenger_code']) && $_POST['mt_messenger_code']!=''){
		// get input
		$updateCode = intval($_POST['mt_messenger_code']);
    // save the option
  ...
  <!-- tiny little static inline style to make it centeralized !-->
    ...
    <!-- client side restrictions for non-script-kidies !-->
		Facebook Page ID: <input type='number' placeholder="numeric value only" name='mt_messenger_code' value='<?=$content?>' />
    <!-- save !-->
